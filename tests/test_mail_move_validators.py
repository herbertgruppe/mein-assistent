...
class TestLenaMailMove(unittest.TestCase):
    """Smoke-test the lena_mail_move endpoint function directly (HBE-1040 fix: POST /move)."""
    ...
    def test_happy_path_calls_post_move_with_destination_id(self):
        ...
        # Response must expose the new message ID returned by Graph (Move = new object at destination)
        self.assertEqual(result.message_id, "AAMkNEWID456")
        self.assertEqual(result.folder, "Archiv")

        # Critical: must use POST …/move with destinationId, NOT PATCH with parentFolderId
    ...
    def test_happy_path_fallback_to_original_id_when_no_new_id(self):
        ...
        req = self._make_req()
        folder_resp = self._make_resp(200, {"id": "FOLDER_ID"})
        move_resp = self._make_resp(200, {})  # no 'id' field in body
